...
export default function EditBudgetScreen() {
    ...
    async function loadData() {
        try {
            const [nsRes, catRes] = await Promise.all([
                nganSachAPI.getById(Number(id)),
                danhMucAPI.getByLoai('CHI'),
            ]);
            const ns = nsRes.data;
            setThang(ns.thang);
            setNam(ns.nam);
            setSoTienGioiHan(String(ns.soTienGioiHan));
            setCategories(catRes.data);

            if (ns.chiTiet && ns.chiTiet.length > 0) {
                setChiTiet(ns.chiTiet.map((ct: any) => ({
                    maDanhMuc: ct.maDanhMuc,
                    tenDanhMuc: ct.tenDanhMuc,
                    soTienDuKien: String(ct.soTienDuKien || 0),
                    ghiChu: ct.ghiChu || '',
                })));
            }
        } catch (err: any) {
    ...
    async function handleSave() {
        ...
        setLoading(true);
        try {
            await nganSachAPI.update(Number(id), {
                thang, nam,
        ...
        <View style={s.container}>
            ...
            >
                <View style={s.headerRow}>
                    ...
                    <Text style={s.headerTitle}>Sửa ngân sách</Text>
                    <View style={{ width: 40 }} />
                </View>
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
                <ScrollView contentContainerStyle={s.form} keyboardShouldPersistTaps="handled">
                    <View style={s.card}>
                        ...
                        {soTienGioiHan ? <Text style={s.moneyDisplay}>{Number(soTienGioiHan).toLocaleString('vi-VN')}đ</Text> : null}
                    </View>

                    <View style={s.card}>
                        <Text style={s.cardTitle}>📊 Phân bổ theo danh mục</Text>
                        {categories.map(dm => {
...
const s = StyleSheet.create({
    ...
    moneyInputWrap: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.inputBg, borderWidth: 2, borderColor: Colors.inputBorder, borderRadius: 14, paddingHorizontal: 16, height: 56 },
