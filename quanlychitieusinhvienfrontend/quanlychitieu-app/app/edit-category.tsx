...
export default function EditCategoryScreen() {
    ...
    async function loadCategory() {
        try {
            const res = await danhMucAPI.getById(Number(id));
            const data = res.data;
            setTenDanhMuc(data.tenDanhMuc);
            setLoaiDanhMuc(data.loaiDanhMuc);
            setMoTa(data.moTa || '');
        } catch (err: any) {
    ...
    async function handleSave() {
        ...
        setLoading(true);
        try {
            await danhMucAPI.update(Number(id), {
    ...
    return (
        <View style={s.container}>
            ...
            >
                ...
                <Text style={s.headerTitle}>Sửa danh mục</Text>
                <View style={{ width: 40 }} />
            </LinearGradient>
                ...
                <ScrollView contentContainerStyle={s.form} keyboardShouldPersistTaps="handled">
                    <View style={s.card}>
                        ...
                        <View style={s.toggleRow}>
                            <TouchableOpacity
                                style={[s.toggleBtn, loaiDanhMuc === 'CHI' && s.toggleBtnActiveChi]}
                                onPress={() => setLoaiDanhMuc('CHI')}
                            >
                                <Text style={{ fontSize: 18 }}>💸</Text>
                                <Text style={[s.toggleText, loaiDanhMuc === 'CHI' && { color: Colors.expense }]}>
                                    Chi tiêu
                                </Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                style={[s.toggleBtn, loaiDanhMuc === 'THU' && s.toggleBtnActiveThu]}
                                onPress={() => setLoaiDanhMuc('THU')}
                            >
                                <Text style={{ fontSize: 18 }}>💰</Text>
                                <Text style={[s.toggleText, loaiDanhMuc === 'THU' && { color: Colors.income }]}>
                                    Thu nhập
                                </Text>
                            </TouchableOpacity>
                        </View>
...
const s = StyleSheet.create({
    ...
    toggleBtnActiveChi: { borderColor: Colors.expense, backgroundColor: Colors.expenseBg },
    toggleBtnActiveThu: { borderColor: Colors.income, backgroundColor: Colors.incomeBg },
    toggleText: { fontSize: 15, fontWeight: '600', color: Colors.textMedium },
    inputWrap: {
        flexDirection: 'row',
        alignItems: 'center',
        gap: 10,
