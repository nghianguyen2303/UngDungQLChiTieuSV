...
export default function EditTransactionScreen() {
    ...
    async function loadTransaction() {
        try {
            const res = await giaoDichAPI.getById(Number(id));
            const data = res.data;
            setLoai(data.loaiGiaoDich);
            setSoTien(String(data.soTien));
            setGhiChu(data.ghiChu || '');
            setNgay(data.ngayGiaoDich ? data.ngayGiaoDich.substring(0, 10) : '');
            if (data.maDanhMuc) {
                setSelectedDM({ maDanhMuc: data.maDanhMuc, tenDanhMuc: data.tenDanhMuc, loaiDanhMuc: data.loaiGiaoDich });
            }
        } catch (err: any) {
            Alert.alert('Lỗi', err.message, [{ text: 'OK', onPress: () => router.back() }]);
        } finally {
    ...
    async function handleSave() {
        ...
        setLoading(true);
        try {
            await giaoDichAPI.update(Number(id), {
        ...
        <View style={s.container}>
            ...
            >
                <View style={s.headerRow}>
                    ...
                    <Text style={s.headerTitle}>Sửa giao dịch</Text>
                    <View style={{ width: 40 }} />
                </View>
                ...
                <View style={s.moneyWrap}>
                    ...
                    <View style={s.moneyRow}>
                        <TextInput
                            ...
                            value={soTien}
                            onChangeText={(t) => setSoTien(t.replace(/[^0-9]/g, ''))}
                        />
                        <Text style={s.moneyCurrency}>đ</Text>
                    </View>
                    {soTien ? (
                        <Text style={s.moneyDisplay}>{Number(soTien).toLocaleString('vi-VN')} VNĐ</Text>
                    ) : null}
                </View>
            </LinearGradient>
                    ...
                    <View style={s.card}>
                        ...
                        <View style={s.inputWrap}>
                            ...
                            <TextInput
                                ...
                                value={ngay} onChangeText={setNgay}
                            />
                        </View>
                        ...
                        <View style={[s.inputWrap, { height: 80, alignItems: 'flex-start', paddingTop: 12 }]}>
                            ...
                            <TextInput
                                ...
                                placeholderTextColor={Colors.textMuted}
                                value={ghiChu} onChangeText={setGhiChu}
                ...
                <View style={s.modalOverlay}>
                    <View style={s.modalContent}>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }) => (
                                <TouchableOpacity
                                    style={[s.modalItem, selectedDM?.maDanhMuc === item.maDanhMuc && s.modalItemActive]}
                                    onPress={() => { setSelectedDM(item); setShowPicker(false); }}
                                >
                            ...
                            ListEmptyComponent={<Text style={s.modalEmpty}>Không có danh mục nào</Text>}
                        />
                    </View>
                </View>
            </Modal>
...
const s = StyleSheet.create({
    ...
    toggleRow: { flexDirection: 'row', gap: 10, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 14, padding: 4, marginBottom: 20 },
    ...
    moneyInput: { fontSize: 40, fontWeight: '800', color: '#fff', minWidth: 80, textAlign: 'center' },
    ...
    label: { fontSize: 13, fontWeight: '600', color: Colors.textMedium, marginBottom: 8, marginLeft: 2, marginTop: 4 },
    ...
    selectWrap: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: Colors.inputBg, borderWidth: 2, borderColor: Colors.inputBorder, borderRadius: 14, paddingHorizontal: 14, height: 52, marginBottom: 18 },
    ...
    modalEmpty: { textAlign: 'center', color: Colors.textMuted, fontSize: 14, marginTop: 20 },
});