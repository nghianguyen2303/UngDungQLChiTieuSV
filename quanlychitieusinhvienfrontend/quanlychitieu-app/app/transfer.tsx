...
export default function EditWalletScreen() {
    ...
    async function loadWallet() {
        try {
            const res = await viAPI.getById(Number(id));
            const d = res.data;
            setTenVi(d.tenVi);
            setLoaiVi(d.loaiVi);
            setSoDu(String(d.soDu || 0));
            setMoTa(d.moTa || '');
        } catch (err: any) {
    ...
    async function handleSave() {
        ...
        setLoading(true);
        try {
            await viAPI.update(Number(id), {
                tenVi: tenVi.trim(),
                loaiVi,
                soDu: Number(soDu) || 0,
                moTa: moTa.trim(),
            });
            Alert.alert('Thành công', 'Cập nhật ví thành công!', [
        ...
        <View style={s.container}>
            ...
            >
                <View style={s.headerRow}>
                    ...
                    <Text style={s.headerTitle}>Sửa ví</Text>
                    <View style={{ width: 40 }} />
                </View>
            </LinearGradient>
                ...
                <ScrollView contentContainerStyle={s.form} keyboardShouldPersistTaps="handled">
                    <View style={s.card}>
                        ...
                        <View style={s.typeRow}>
                            {walletTypes.map(t => (
                                <TouchableOpacity key={t.value} style={[s.typeBtn, loaiVi === t.value && s.typeBtnActive]} onPress={() => setLoaiVi(t.value)}>
                        ...
                        <View style={s.inputWrap}>
                            ...
                            <TextInput style={s.input} placeholder="Tên ví" placeholderTextColor={Colors.textMuted} value={tenVi} onChangeText={setTenVi} />
                        </View>

                        <Text style={s.label}>Số dư</Text>
                        <View style={s.inputWrap}>
                            <Ionicons name="cash-outline" size={20} color={Colors.textMuted} />
                            <TextInput style={s.input} placeholder="0" placeholderTextColor={Colors.textMuted} keyboardType="numeric" value={soDu} onChangeText={t => setSoDu(t.replace(/[^0-9]/g, ''))} />
                        ...
                        <View style={[s.inputWrap, { height: 80, alignItems: 'flex-start', paddingTop: 12 }]}>
                            ...
                            <TextInput style={[s.input, { textAlignVertical: 'top' }]} placeholder="Ghi chú..." placeholderTextColor={Colors.textMuted} value={moTa} onChangeText={setMoTa} multiline numberOfLines={3} />
                        </View>

                        <TouchableOpacity onPress={handleSave} disabled={loading} activeOpacity={0.85}>
                            <LinearGradient colors={[Colors.primary, Colors.secondary]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[s.btn, loading && { opacity: 0.7 }]}>
